  ...
  for ratio in "${whitelist_blacklist_ratios[@]}"; do
    for lr in "${learning_rates[@]}"; do
      for rank in "${lora_ranks[@]}"; do
        ...
        echo "Starting run with LR=${lr}, SAMPLES=${samples}, RATIO=${ratio}, RANK=${rank}"
        echo "Train dataset: $train_dataset"
        echo "Test dataset: $test_dataset"
        
        # Create a specific config file for this run
        config_file="yaml/refusal-llama-sft-${lr}-${samples}-${ratio}-${rank}.yaml"
        ...
        sed -i "s|# lora_rank will be set in the sweep|lora_rank: $rank|g" "$config_file"
